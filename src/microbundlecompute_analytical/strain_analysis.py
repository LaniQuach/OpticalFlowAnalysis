...
def compute_sub_domain_position_strain_all(tracker_row_all: List, tracker_col_all: List, sd_box_list: List) -> List:
    ...
    for sd_box in sd_box_list:
        ...
        sd_row = []
        sd_col = []
        for kk in range(0, num_beats): #,numbeats
        ...
        # print("sub domain f all", sub_domain_F_all)
...
def F_to_E_all(sub_domain_F_rr_all: List, sub_domain_F_rc_all: List, sub_domain_F_cr_all: List, sub_domain_F_cc_all: List) -> List:
    ...
    for kk in range(0, num_beats):
        num_frames = 1
        Ecc_arr = np.zeros((num_sub_domains))
        Ecr_arr = np.zeros((num_sub_domains))
        Err_arr = np.zeros((num_sub_domains))
        for jj in range(0, num_sub_domains):
            for ii in range(0, num_frames):
                F_rr = sub_domain_F_rr_all[kk][jj,1] #,ii
                F_rc = sub_domain_F_rc_all[kk][jj,1]
                F_cr = sub_domain_F_cr_all[kk][jj,1]
                F_cc = sub_domain_F_cc_all[kk][jj,1]
                Ecc_arr[jj], Ecr_arr[jj], Err_arr[jj] = F_to_E(F_rr, F_rc, F_cr, F_cc)
...
def pngs_sub_domain_strain(
    ...
    for beat in range(0, num_beats):
        ...
        for kk in range(0,2):
            ...
            jj = kk - start_idx
            tracker_col = np.nan_to_num(tracker_col)
            tracker_row = np.nan_to_num(tracker_row)

            plt.scatter(tracker_col[:, kk], tracker_row[:, kk], c=E[:], s=50, cmap=col_map, vmin = -0.085, vmax = 0.01)
...
def visualize_sub_domain_strain(
    ...
    sub_domain_F_rr_all, sub_domain_F_rc_all, sub_domain_F_cr_all, sub_domain_F_cc_all, sub_domain_row_all, sub_domain_col_all, info, strain_info = load_sub_domain_strain(folder_path, fname = "elastix_")
