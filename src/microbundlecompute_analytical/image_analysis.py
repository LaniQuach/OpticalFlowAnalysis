...
def track_all_steps(img_list_uint8: List, mask: np.ndarray,feature_params: dict, lk_params: dict) -> np.ndarray:
    ...
    np.save('tracking_points.npy', track_points)
    #find points to track based on the first image
    #track points returns (number of points [1,2,3,4...], row loc, col loc)
...
def trackPoints(img_list_uint8: List, mask: np.ndarray,feature_params: dict) -> np.ndarray:
    img_0 = img_list_uint8[0]
    track_points = mask_to_track_points(img_0, mask, feature_params)
    return track_points
...
# find out what the input is for hibas strain code
# get displacement poitns of analytical and elastix results to the points used by hibas code 
# then run hibas strian code on the elastix and analytical displacement arryas 
...
def run_tracking(folder_path: Path, fps: int, length_scale: float, elastix_x, elastix_y) -> List:
...
def run_visualization(folder_path: Path, automatic_color_constraint: bool = True, col_min_abs: Union[int, float] = 0, col_max_abs: Union[int, float] = 8, col_min_row: Union[int, float] = -3, col_max_row: Union[int, float] = 4.5, col_min_col: Union[int, float] = -3, col_max_col: Union[int, float] = 4.5, col_map: object = plt.cm.viridis) -> List:
    ...
    # tracker_row_all, tracker_col_all, info, _ = load_tracking_results(folder_path=folder_path, fname = "elastix_")
